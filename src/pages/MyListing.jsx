        ...
        fetch(`https://roommate-server-side-alpha.vercel.app/addListing/${id}`, {
          ...
          .then((data) => {
            ...
            if (data.deletedCount) {
              ...
              const remainingMyListing = myListing.filter(
          ...
          <tbody>
            ...
            {myListing.map((data) => (
              <tr key={data._id}>
                <th>
                  <div>
                    <h1 className="lg:text-xl text-sm">{data.title?.length > 12 ? data.title.slice(0, 20) + "..." : data.title}</h1>
                ...
                <td>
                  <h1 className="bg-green-700 rounded-full text-center py-1 w-5/6 text-white text-xs">
                    {data.availability}
                  </h1>
                </td>
